import { useContext } from 'react'
import Cards from './Cards'
import { Link } from 'react-router-dom'
import { myContext } from '../Context/Context'

const Course = () => {
  const { book, setbook } = useContext(myContext)

  return (
    <div className=' max-w-screen-2xl container mx-auto md:px-20 px-4'>
      <div className='pt-28 items-center justify-center text-center'>
        <h1 className='text-2xl font-semibold md:text-4xl'>We're delighted to have you
          <span className='text-pink-500'> Here! :)</span></h1>
        <p className='mt-12'>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Cum veritatis expedita ut sequi quasi aspernatur, aut, eaque, commodi sed perspiciatis optio qui placeat nulla cumque tempore fugit quam obcaecati unde officia maiores beatae quos excepturi modi. Reiciendis, inventore pariatur? Omnis.
        </p>
        <button className='mt-6 bg-pink-500 text-white px-4 py-2 rounded-md hover:bg-pink-700 duration-300'><Link to="/">Back</Link></button>
      </div>
      <div className='mt-12 grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 '>
        
        {
          book && book.map((ele)=>{
            return <>
                <Cards item={ele} key={ele._id}/>
            </>
          })
        }
      </div>
    </div>
  )
}

export default Course